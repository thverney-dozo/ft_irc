...
/*   Updated: 2021/03/30 17:11:20 by thverney         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Server::Server(const char *local_port, std::string const &local_password, std::string	const &host_ip,
			   const char *host_port, std::string host_password)
{
	this->local_port = (char*)local_port;
	this->local_password = local_password;

	if (host_ip != "NULL" && host_port != "NULL")
	{
		this->host_ip = host_ip;
		this->host_port = (char*)host_port;
		if (host_password != "NULL")
			this->host_password = host_password;
	}

    setup_server(this->local_port);
	init_commands();
}
...
void     Server::setup_server(char *local_port)
{
	...
	serv_adr.sin_port = htons(atoi(local_port)); 		    // Server will listen to this local_port
